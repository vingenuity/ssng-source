class SSG_Attachment_Base extends Actor
	abstract
	dependson(SSG_Pawn);
...
var class<UDKWeapon> WeaponClass;
 ...
 * to the Mesh.
 */
simulated function AttachTo(SSG_Pawn OwnerPawn)
{
...
simulated function PlayImpactEffects(vector HitLocation)
{
	...
	if ( (P != None) && EffectIsRelevant(HitLocation, false, MaxImpactEffectDistance) )
	{
		...
		//{
		//	UTVehicle(V).ApplyMorphDamage(HitLocation, WeaponClass.Default.InstantHitDamage[SSG_Pawn(Owner).FiringMode], WeaponClass.Default.InstantHitMomentum[SSG_Pawn(Owner).FiringMode]*FireDir);
		//}
...
simulated function CheckBulletWhip(vector FireDir, vector HitLocation)
{
	...
	//
	//ForEach LocalPlayerControllers(class'SSG_PlayerController', PC)
	//{
...
/*simulated function SetWeaponOverlayFlags(SSG_Pawn OwnerPawn)
{
